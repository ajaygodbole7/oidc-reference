...
local function refresh_session(conf, sid)
  ...
  if status == 404 or status == 409 then
    -- Session was logged out / refresh-reuse detected. Auth Service has
...
function _M.access(conf, ctx)
  ...
  -- Step 6: header shaping. Strip inbound Cookie + hop-by-hop, inject
  -- the bearer. core.request.set_header sets BOTH ctx headers (visible
  -- to later plugins) and the upstream request.
