...
#                              (needs Auth Service + Keycloak realm import)
...
test_expiring_session_triggers_refresh_delegation() {
  ...
  if [ "${RUN_REFRESH_TESTS:-0}" != "1" ]; then
    skip_test "$name" "set RUN_REFRESH_TESTS=1 (needs Auth Service + Keycloak realm import)"
    return 0
  fi
  sid="expiring-1"
  setup_session "$sid" "test-jwt-old" 30 "unused.csrf"
  before="$(get_session_field "$sid" access_token)"
  status="$(curl -s -o "$BODY_TMP" -w '%{http_code}' \
    -H "Cookie: __Host-sid=$sid" \
    "$GATEWAY_BASE/api/me" 2>/dev/null || true)"
  # Gateway should have triggered /internal/refresh which rotates the token.
  after="$(get_session_field "$sid" access_token)"
  case "$status" in
    200|401)
      printf '[PASS] %s forward_completed (status=%s)\n' "$name" "$status"
      PASSED=$((PASSED + 1))
      ;;
    *)
  ...
  if [ -n "$before" ] && [ "$before" != "$after" ]; then
    printf '[PASS] %s token_rotated (before!=after)\n' "$name"
    PASSED=$((PASSED + 1))
  else
    printf '[FAIL] %s expected access_token to change after refresh; before=%s after=%s\n' \
      "$name" "$before" "$after"
    FAILED=$((FAILED + 1))
  fi
  clear_session "$sid"
...
test_cookie_strip_does_not_leak_to_upstream() {
  ...
  # The RS does not echo request headers in its response body; without an
  # in-test mock RS or a request-header logger we cannot prove the strip
  # from a shell harness. Defense-in-depth: covered by plugin unit tests
  # (Lua side); verified manually via the apisix access log.
  skip_test "$name" "requires RS-side header echo or apisix access-log inspection; manually verified"
}

test_query_string_preserved() {
  name="query_string_preserved"
  skip_test "$name" "requires RS-side query-string echo endpoint; out of scope for this harness"
}

test_hop_by_hop_headers_stripped() {
  name="hop_by_hop_headers_stripped"
  skip_test "$name" "requires RS-side header echo to assert strip; out of scope for this harness"
}
