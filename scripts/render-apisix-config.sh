...
missing=
for var in GATEWAY_CLIENT_SECRET CSRF_SIGNING_KEY; do
  eval "value=\${$var:-}"
  if [ -z "$value" ]; then
    missing="$missing $var"
  fi
...
envsubst '${GATEWAY_CLIENT_SECRET} ${CSRF_SIGNING_KEY}' \
  < "$TEMPLATE" > "$RENDERED"
...
for key in gateway_client_secret cookie_signing_key; do
