...
#   REALM_FILE               Realm JSON to statically verify. Default realm/oidc-reference-realm.json.
#   EXPECTED_REALM           Expected realm name. Default oidc-reference.
#   EXPECTED_API_AUDIENCE    Expected Resource Server audience. Default oidc-reference-api.
...
realm_file="${REALM_FILE:-$project_dir/realm/oidc-reference-realm.json}"
expected_realm="${EXPECTED_REALM:-oidc-reference}"
expected_api_audience="${EXPECTED_API_AUDIENCE:-oidc-reference-api}"
