...
| B-2 | CSRF on state-changing `/api/**` | Signed double-submit: `XSRF-TOKEN` carries `<value>.<HMAC-SHA256(value)>`. The SPA echoes it as `X-XSRF-TOKEN`; both sides validate signature with the shared key. | An attacker with `document.cookie` write on a sibling subdomain cannot forge a valid signature; pre-XSS access to the page can still issue same-origin requests (B-1). | ADR §B4 |
...
| S-2 | Refresh-token reuse | Realm: `revokeRefreshToken: true`, `refreshTokenMaxReuse: 0`. `InternalRefreshController` surfaces Keycloak's `invalid_grant` as a 409 + `DEL sess:{sid}` + audit event. | None within the rotation contract. | RFC 9700 §4.14 |
...
| S-4 | Session lives past intended ceiling | `SessionRecord.absoluteExpiresAt` enforces a 12 h hard cap. Both `AuthController#session` and `InternalRefreshController` `DEL sess:{sid}` when the ceiling is crossed (including the race window during a refresh round-trip). | None within the contract. | SPEC-0001 §7.2 |
...
| # | Threat | Mitigation | Residual | Reference |
|---|---|---|---|---|
| D-1 | Local-dev sentinel secrets ship to production | `SecretSentinelValidator` (Java) refuses to start in `prod` / `production` Spring profile if `AUTH_CLIENT_SECRET` contains the `CHANGE_BEFORE_DEPLOY` marker or `APP_COOKIE_SIGNING_KEY` is the known dev base64. Mirror guard in `bff-session.lua` emits a WARN at plugin load when its config carries either sentinel. | Production deploys MUST set the env vars explicitly. The fail-fast is profile-gated; a deployment with no Spring profile set would only WARN, not fail. | `SecretSentinelValidator` |
...
| `refresh_rejected` | Pre-refresh validation failed | `missing_sid`, `no_such_session`, `session_absolute_expired`, `session_absolute_expired_post_refresh` |
| `refresh_failed` | Keycloak unreachable or other transient error | `authorization_server_unreachable` |
| `refresh_token_reuse` | Keycloak returned `invalid_grant` on refresh | `session_invalidated` |
