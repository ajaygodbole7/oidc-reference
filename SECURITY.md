...
| S-4 | Session lives past intended ceiling | `SessionRecord.absoluteExpiresAt` enforces an 8 h hard cap (kept ≤ the IdP SSO max session lifespan; Keycloak default 10 h). Both `AuthController#session` and `InternalResolveController` `DEL sess:{sid}` when the ceiling is crossed (including the race window during a refresh round-trip). | None within the contract. | SPEC-0001 §7.2 |
