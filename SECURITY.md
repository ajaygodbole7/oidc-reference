...
| S-5 | Sub-session fixation: attacker who observed the sid cookie keeps it valid across token refreshes | The `sid` rotates on every refresh (A6). A successful refresh mints a new `sid`, atomically moves `sess:{sid}` → `sess:{sid'}` (only if the old key still exists — a concurrent logout is not resurrected), repoints the `idp_sid:`/`sub_sessions:`/`logout_hint:` indexes, and the gateway re-issues the `__Host-sid` cookie. A short `rotated:{sid}` breadcrumb (30 s) forwards concurrent in-flight requests still holding the old `sid` so they don't lose the session. | A once-observed `sid` is valid only until that session's next refresh (or the brief rotation-grace window), not the absolute-TTL window. Pair with a short access-token lifetime to bound how long a stolen `sid` survives; `app.max-refresh-token-age` additionally bounds non-rotating refresh tokens. | SPEC-0001 §7.2; ADR §F. |
