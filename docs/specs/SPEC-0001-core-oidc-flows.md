...
- **Compose-stack completeness for Auth Service and Resource Server.**
  Both are designed to run via `./mvnw spring-boot:run` for the inner
  loop. Dockerfiles exist; full-Compose mode is intentionally not the
  default to keep the JVM hot-reload loop usable. A `compose.full.yaml`
  overlay file would be a non-disruptive addition if needed.
...
- Redirect URI: `http://127.0.0.1:5173/auth/callback/idp` (Spring's
  `{baseUrl}/auth/callback/{registrationId}` with the redirection endpoint
  base URI overridden to `/auth/callback`; registration name is the
  generic `idp`, not the IdP brand). In the full Compose stack the
  redirect URI is the APISIX-fronted hostname for the same path.
- Post-logout redirect URI: `http://127.0.0.1:5173/` (SPA root via the
  Vite proxy; the dev-mode same-origin pattern lands the user back on
  the SPA). In the full Compose stack the URI is the APISIX-fronted
  hostname for `/`.
...
| Path | Method | Auth | Purpose |
|---|---|---|---|
| `/internal/refresh` | POST | Client Credentials (Bearer JWT, `aud=oidc-reference-auth-internal`, `azp/client_id=oidc-reference-api-gateway`) | Auth Service endpoint, called by the API Gateway. Reachable only on the internal Compose network — never via the browser-facing ingress. Performs the refresh-token grant against Keycloak under a per-session lock, validates rotation, emits the refresh-reuse audit event on `invalid_grant`, and updates `sess:{sid}`. Full contract per §7.1. |
...
- `sess:{sid}` → see §7.2 for the full schema and the tolerant-reader
  contract. Sliding TTL 30m (idle), absolute cap 12h. Written by the Auth
  Service's custom state-store session repository after callback; not
...
- No pre-callback **session** cookie. The Auth Service sets no cookies at
  `/auth/login`; the only pre-auth server-side state is `tx:{state}`
  (keyed by the OAuth `state` parameter). Login-CSRF and session-swapping
...
- The Auth Service sets `XSRF-TOKEN` as a JS-readable cookie (not
  `HttpOnly`) with `Secure`, `SameSite=Lax`, `Path=/`. The cookie value is
...
- Scope and role authorities populated by a custom
  `JwtAuthenticationConverter` that lifts `scope` → `SCOPE_*` and
...
| Token theft via JavaScript | Tokens never reach browser; cookie is `HttpOnly` |
...
`absolute_expires_at` is the hard ceiling (default 12h from `created_at`) past
...
- One documented command starts Keycloak (+ DB), Valkey, Auth Service,
  API Gateway (APISIX), Resource Server, and the SPA.
...
- `/auth/login` with a valid `return_to` issues 302 to the AS with
  ...
  `created_at`. No cookies set. The IdP authorization redirect does not
  contain `return_to` or `saved_request`.
...
- **`/internal/refresh` contract tests (§7.1).** Rejects requests with
  ...
  `sess:{sid}`, returns 200 with new expiry. On Keycloak `invalid_grant`
  (refresh-token reuse), emits the refresh-reuse audit event, deletes
  `sess:{sid}`, and returns 409. On Keycloak transient failure, returns
  502. Per-session lock prevents concurrent duplicate refresh.
...
| `app.roles-claim-path` | `application.yml` | Keycloak: `[realm_access, roles]`. Okta: `[groups]`. Auth0: `[https://your-app/roles]`. Entra: `[roles]`. See PROVIDER-ADAPTERS.md. |
...
- The BFF session shape (sliding 30m, absolute 12h, sid in HttpOnly cookie,
