...
- Redirect URIs: `http://127.0.0.1:5173/auth/callback/idp` in the
  frontend-dev case (Vite proxy fronts the Auth Service so the callback
  lands on the SPA origin). In the full Compose stack the redirect URI is
  the Traefik hostname. Registration name in the path is the generic
  `idp` (not the IdP brand) so swapping IdPs requires no URI change.
- Post-logout redirect URIs: `http://127.0.0.1:5173/`.
- Web origins: none.
