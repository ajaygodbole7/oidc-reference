...
- Refresh tokens rotated on every use; reuse detection emits a structured
  audit event and invalidates the session.
...
- Refresh rotation works; reuse invalidates session and emits the
...
- Reused refresh token at Keycloak invalidates `sess:{sid}` and emits the
  structured refresh-reuse audit event.
- `/internal/refresh` contract:
  ...
  - Returns `409` on refresh-reuse and emits the audit event.
